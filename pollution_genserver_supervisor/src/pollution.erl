...
-export([create_monitor/0, add_station/3, add_value/5, remove_value/4, get_one_value/4, get_station_mean/3, get_daily_mean/3, traverse_keys/2, get_map_keys/1,
    only_one_Coordinates/2, only_one_Name/2, unique_measure/3, get_minimum_pollution_station/2]).
...
% Dodaj do modułu pollution funkcję get_minimum_pollution_staion, która wyszuka stację z najniższym zanieczyszczeniem danego typu. 
get_minimum_pollution_station(Type, Monitor) -> 
    A = [ {Station, MapMeasures} || Station := MapMeasures <- Monitor],
    B = [ {Value, Station} || {Station, MapMeasures} <- A, Measure := Value <- MapMeasures, Measure#measure.type == Type ],
    element(2, lists:min(B)). 
%%
